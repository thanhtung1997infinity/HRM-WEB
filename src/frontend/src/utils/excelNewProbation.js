...
export async function generateNewProbationForm(data) {
  ...
  worksheet.getCell("B1").value = {
    richText: [{ font: ExcelStyles.headerTitle, text: data.templateTitle }],
  };
  //Employee Name
  ...
  worksheet.getCell("G3").value =
    data.newProbation.jobTitle === "Not Found"
      ? "......"
      : data.newProbation.jobTitle;
  //Line Manager
  ...
  worksheet.getCell("G4").value =
    data.newProbation.probationLineManagerName === null
      ? "......"
      : data.newProbation.probationLineManagerName;
  //Period of Appraisal
  ...
  worksheet.getCell("G5").value =
    "From " + data.newProbation.startDay + " to " + data.newProbation.endDay;
  ...
  data.overCmtData.forEach((evaluation) => {
    ...
    worksheet.getCell(commentNameCellName).value =
      evaluation.overall_comment.term;
    //Score
    const score = evaluation.score > 0 ? evaluation.score : 1;
  ...
  worksheet.getCell(startCellName).value = {
    richText: [{ font: ExcelStyles.bodyFont, text: data.newProbation.term }],
  };
